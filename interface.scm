...
;; looks up signal-proc-pairing directly, given a signal, return the associated procedure defined in signal-proc-pairing. returns false when there is no signal to match
...
;; if any input is entered that returns false from find-proc-from-signal (ie it isn't defined in signal-proc-pairing) then display the helpstring and silently resume computing.
(define (handle-input signal state)
  (let ((proc (cadr (find-proc-from-signal signal))))
    (if (not proc)
      (begin
	(display helpstring)
	(silent-try-until-loop))
      (proc state))))
